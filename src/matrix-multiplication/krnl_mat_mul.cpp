...
// std::cout << "\n\ncomputing c for row: "<< A_row[n1].x << ", col: " <<
// B_col[n2].y << "\n" << std::flush;

/* print elem x,y of A_row */
// std::cout << "\nA_row[" << A_row[n1].x <<"][" << A_row[n1].y << "]=(" <<
// A_row[n1].data.real << ", " << A_row[n1].data.imag << "i) ; last in row: " <<
// A_row[n1].last_in_row << "\n" << std::flush;

/* print elem x,y of B_col */
// std::cout << "B_col[" << B_col[n2].x <<"][" << B_col[n2].y << "]=(" <<
// B_col[n2].data.real << ", " << B_col[n2].data.imag << "i) ; last in row: " <<
// B_col[n2].last_in_row << "\n" << std::flush;

/* print elem x,y of c */
// std::cout << "c[" << A_row[n1].x <<"][" << B_col[n2].y << "]=(" <<
// c.data.real << ", " << c.data.imag << "i)\n\n" << std::flush;

/* First row read correctly */
// std::cout << "\nq: " << q << " A_row[" << A_row[q].x <<"][" << A_row[q].y <<
// "]=(" << A_row[q].data.real << ", " << A_row[q].data.imag << "i) ; last in
// row: " <<  A_row[q].last_in_row << "\n" << std::flush;

// Vectors read correcly, both times o = 0
// std::cout << "o: " << o << " B_col[" << B_col[o].x <<"][" << B_col[o].y <<
// "]=(" << B_col[o].data.real << ", " << B_col[o].data.imag << "i) ; last in
// row: " <<  B_col[o].last_in_row << "\n" << std::flush;

// std::cout << "\nCompute c loop iteration: " << std::flush;
// std::cout << "\nA_row[" << n1 << "]=(" << A_row[n1].data.real << ", " <<
// A_row[n1].data.imag << "i) ; last in row: " <<  A_row[n1].last_in_row << " ;
// last in tensor: " <<  A_row[n1].last_in_tensor << "\n" << std::flush;
// std::cout << "B_col[" << n2 << "]=(" << B_col[n2].data.real << ", " <<
// B_col[n2].data.imag << "i) ; last in row: " <<  B_col[n2].last_in_row << " ;
// last in tensor: " <<  B_col[n2].last_in_tensor << "\n" << std::flush;

// of X - val, x, y, lir, lit
// std::cout << "\nX val: " << x.data.real << "," << x.data.imag << "i" <<
// std::flush; std::cout << "\nx: " << x.x << std::flush; std::cout << "\ny: "
// << x.y << std::flush; std::cout << "\nl_i_r: " << x.last_in_row <<
// std::flush; std::cout << "\nl_i_t: " << x.last_in_tensor << std::flush;

using namespace Complex;

void matrix_multiplication(cmplx_t *Ad, coo_meta_t *Am, cmplx_t *Bd,
                           coo_meta_t *Bm, cmplx_t *Cd, coo_meta_t *Cm,
                           rank_t A_R, dim_t A_NZ, dim_t B_NZ, dim_t *CD) {
  // clang-format off
...
#pragma HLS INTERFACE s_axilite port=return
  ...
  // Store result
  Tensor::store(C_stream, Cd, Cm, *CD); // FIX how to pass dimension of c stream
}
...
/* ASSUMPTIONS
- since we check for rows/cols on last_in_row, we can skip cleaning the arrays
since the relevant data will be overwritten
- since we can't read and write to same stream in same loop, A/B_stream_temp are
used to temporarely store the matrix A or B
*/
...
LOOP_T: /* iterate over A rows */
  ...
  LOOP_Q: /* read A_row */
    ...
    for (;;) {
      ...
      // std::cout << "(" << A_row[q].data.real << "," << A_row[q].data.imag <<
      // "i) -> " << std::flush;
  ...
  LOOP_P: /* iterate over B cols */
    ...
    LOOP_O: /* read B_col */
      ...
      for (;;) {
        ...
        // std::cout << "(" << B_col[0].data.real << "," << B_col[0].data.imag
        // << "i) -> " << std::flush;
    ...
    LOOP_N: /* compute c = A_row * B_col */
      ...
      for (;;) {
        ...
        if (A_row[n1].y == B_col[n2].x) { /* if same index multiply */
          ...
          // std::cout << "computing c[" << A_row[n1].x << "][" << B_col[n2].y
          // << "] w/ current values: " << "\n" << std::flush; std::cout <<
          // "A_row[" << A_row[n1].x <<"][" << A_row[n1].y << "]=(" <<
          // A_row[n1].data.real << ", " << A_row[n1].data.imag << "i) ; last in
          // row: " <<  A_row[n1].last_in_row << "\n" << std::flush; std::cout
          // << "B_col[" << B_col[n2].x <<"][" << B_col[n2].y << "]=(" <<
          // B_col[n2].data.real << ", " << B_col[n2].data.imag << "i) ; last in
          // row: " <<  B_col[n2].last_in_row << "\n" << std::flush;
          //// What are the state of n1, n2
          // std::cout << "n1: " << n1 << ", n2: " << n2 << "\n" << std::flush;
          //// What is the current value of c_tmp
          // std::cout << "(" << c_tmp.data.real << ", " << c_tmp.data.imag <<
          // "i)" << "\n" << std::flush;
        ...
        if ((A_row[n1].last_in_row && B_col[n2].x >= A_row[n1].y) ||
            (B_col[n2].last_in_row &&
             A_row[n1].y >= B_col[n2].x)) { /* break if last in row */
      ...
      // std::cout << "\n\nfinal c[" << A_row[n1].x << "][" << B_col[n2].y << "]
      // = " << c.data.real << "," << c.data.imag << "i" << std::flush;
      // std::cout << "\nlast y of A_row: " << A_row[n1].y << std::flush;
      // std::cout << "\nlast x of B_col: " << B_col[n2].x << "\n\n\n\n" <<
      // std::flush;

      if (!(c.data.real == 0.0f && c.data.imag == 0.0f)) { /* update c */
        // std::cout << "\n\033[1;31mFINAL COMPUTED C \033[0m[" << A_row[n1].x
        // << "," << B_col[n2].y << "]=(" << c.data.real << "," << c.data.imag
        // << "i)\n" << std::flush;
        c.x = A_row[n1].x;
        c.y = B_col[n2].y;
        if (!(old_c.data.real == 0.0f && old_c.data.imag == 0.0f)) {
          ...
          // std::cout << "\nwrote on stream old_c: " << old_c.data.real << ","
          // << old_c.data.imag << "i" << std::flush; std::cout << "\nx: " <<
          // old_c.x << std::flush; std::cout << "\ny: " << old_c.y << "\n\n" <<
          // std::flush;
          (*CD)++;
        }
        old_c = c;
        //// of (*old_c)- val, x, y, lir, lit
        ////std::cout << "\n(*old_c) val: " << (*old_c).data.real << "," <<
        ///(*old_c).data.imag << "i" << std::flush; /std::cout << "\nx: " <<
        ///(*old_c).x << std::flush; /std::cout << "\ny: " << (*old_c).y <<
        /// std::flush;
      }
  ...
  if (!(old_c.data.real == 0.0f &&
        old_c.data.imag == 0.0f)) { /* update last in tensor for c */
    old_c.last_in_row = 1;
