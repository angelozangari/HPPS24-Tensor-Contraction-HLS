...
% TODO Describe the FPGA kernel’s architecture and how it accelerates quantum simulations.
...
Both kernels are designed to operate on matrices represented in the Coordinate (COO) sparse format, optimizing memory usage and computational efficiency for the typically sparse quantum operators. As show in the (name)\cite{FormatsHwSpMsurvey} paper, the COO format allows for storing a sparse matrix by using for each non-zero value three elements, its row, its column and the value itself. This is particularly important for the FPGA kernels because by storing the element in this format we can have the tensors both in row major and in column major by simply ordering them by the appropriate index (may it be row or column). 
